    ...
    public class LoginHandler
    {
            ...
            return new OperationResult.SeeOther
                       {
                           RedirectLocation =
                               new Uri(
                               "https://www.facebook.com/dialog/oauth?client_id=149734561800297&redirect_uri=http://localhost:62571/login&scope=read_stream,user_interests,user_education_history,user_birthday,user_about_me,user_activities,offline_access")
                       };
        ...
        public object Get(Guid registrationId, string afterLogin)
        {
            ...
            return new OperationResult.SeeOther
            {
                RedirectLocation =
                    new Uri(
                    "https://www.facebook.com/dialog/oauth?client_id=149734561800297&scope=read_stream,user_interests,user_education_history,user_birthday,user_about_me,user_activities,offline_access&redirect_uri=http://localhost:62571/login")
            };
        ...
        private static void LogIn(string code, string registrationId)
        {
            ...
            if (FacebookOAuthResult.TryParse(url, out result))
            {
                if (result.IsSuccess)
                {
                    cl.RedirectUri = new UriBuilder("http://localhost:62571/login").Uri;
                    ...
                    dynamic accessTokenResponse =
                        cl.ExchangeCodeForAccessToken(code,
                                                      new Dictionary<string, object>
                                                          {
                                                              {"redirect_uri", "http://localhost:62571/login"}
                                                          });
